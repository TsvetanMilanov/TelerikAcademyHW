namespace ExistingPath
{
    ...
    {
        private static Queue<MatrixCellCoordinates> foundPath = new Queue<MatrixCellCoordinates>();
        ...
        public static void Main()
        {
            ...
            matrix[endPointRow, endPointCol] = "e";

            result = MoveInDirection(result, 2, 1, new Queue<MatrixCellCoordinates>(), Directions.None, false);

            Console.WriteLine("Found path:");

            Console.WriteLine();
            var matrixWithCurrentPath = FillPath(matrix, foundPath);
            PrintMatrix(matrixWithCurrentPath);
            Console.WriteLine();
        }
        ...
        private static string[,] MoveInDirection(string[,] matrix, int rowPosition, int colPosition, Queue<MatrixCellCoordinates> currentPath, Directions lastDirection, bool pathIsFound)
        {
            if (pathIsFound)
            {
                return matrix;
            ...
            if (!positionIsInRangeOfTheMatrix || matrix[rowPosition, colPosition] == "e")
            {
                if (positionIsInRangeOfTheMatrix && matrix[rowPosition, colPosition] == "e")
                {
                    foundPath = new Queue<MatrixCellCoordinates>(currentPath);
                    pathIsFound = true;
                }
            ...
            {
                matrix = MoveInDirection(matrix, rowPosition + 1, colPosition, path, Directions.Down, pathIsFound);
            ...
            if (lastDirection != Directions.Left && !currentPath.Contains(nextCoordinates))
            {
                ...
                matrix = MoveInDirection(matrix, rowPosition, colPosition + 1, path, Directions.Right, pathIsFound);
                matrix[rowPosition, colPosition] = "0";
            }
            ...
            {
                path = new Queue<MatrixCellCoordinates>(currentPath);
                matrix = MoveInDirection(matrix, rowPosition, colPosition - 1, path, Directions.Left, pathIsFound);
                matrix[rowPosition, colPosition] = "0";
            }
            ...
            {
                path = new Queue<MatrixCellCoordinates>(currentPath);
                matrix = MoveInDirection(matrix, rowPosition - 1, colPosition, path, Directions.Up, pathIsFound);
                matrix[rowPosition, colPosition] = "0";
            }
